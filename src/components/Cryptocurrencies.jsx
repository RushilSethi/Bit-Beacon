        ...
        {cryptos?.map((currency, id)=>{
          return <Col xs={24} sm={12} lg={6} className='crypto-card' key={currency.id}>
            <Link to={`/cryptos/${currency.uuid}`}>
              <Card className="crypto-card-styler"  title={`${currency.rank}. ${currency.name}`} 
                extra={<img className='crypto-img' src={currency.iconUrl} alt='coin icon' style={{ width: 40, height: 40 }}/>} 
                hoverable>
